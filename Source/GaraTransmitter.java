...
public class GaraTransmitter implements ReceiveStreamListener, RemoteListener,
    ControllerListener
{
    ...
    private Garatron garatron;

    public GaraTransmitter( Garatron garatron, int data_port)
    {
		this.garatron= garatron;
    ...
    {
		// (not checked)Should have checked the type.
		...
		for (int i = 0; i < pbss.length; i++)
		{
			...
			try
			{
				...
				{
					GaraTarget target= (GaraTarget) targets.elementAt( k);
	...
	public void addTarget( int localPort, RTPManager mgr, String ip, int port)
	{
		try
		{
			...
			garatron.addTargetToList( localPort + "", ip, port + "");
		}
		catch( Exception e)
		{
			e.printStackTrace();
	...
	public void update( ReceiveStreamEvent event)
	{
		...
		//garatron.rtcpReport( sb.toString());
	}
